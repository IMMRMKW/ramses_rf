...
class Frame:
    ...
    def __init__(self, frame: str) -> None:
        ...
        # Handle optional ': ' prefix or leading space
        if frame.startswith(": "):
            # ": RQ --- ..." format - fields are [":","RQ","---",...]
            verb_idx, seqn_idx, code_idx, len_idx, payload_idx, addr_start = (
                1,
                2,
                6,
                7,
        ...
        self.verb: VerbT = fields[verb_idx]  # type: ignore[assignment]
        self.seqn: str = fields[seqn_idx]
        self.code: Code = fields[code_idx]  # type: ignore[assignment]
        self.payload: PayloadT = fields[payload_idx]

        # Auto-correct length field to match actual payload (trust the device, not the length field)
        actual_len = len(self.payload) // 2
        declared_len_str = fields[len_idx]
        ...
        try:
            # Get address field range based on format
            self.src, self.dst, *self._addrs = pkt_addrs(  # type: ignore[assignment]
                " ".join(fields[i] for i in range(addr_start, addr_start + 3))
            )
