...
def parser_2411(payload: str, msg: Message) -> dict[str, Any]:
    ...
    # Handle both known HVAC parameters and unknown device parameters (like CO2 sensors)
    parameter = payload[4:6]
    if parameter in _2411_TABLE:
        # Known HVAC parameter
        description = _2411_TABLE[parameter]
        param_type = "hvac_parameter"
    else:
        # Unknown parameter (e.g., from CO2 sensors, pairing messages, etc.)
        _LOGGER.info(
            f"Unknown 2411 parameter {parameter} (likely from non-HVAC device)"
        )
        description = f"Unknown device parameter {parameter}"
        param_type = "unknown_parameter"
